...
outputdir <- "C:/Users/dag71/Dropbox/SynergiesTradeoffs.Review/Data extraction/updated_plots/"
...
# Author affliation
data %>% group_by(auth_aff) %>% summarise(num=n_distinct(aid), pct=num/75)
...
(pInt_typ.waffle <- waffle(int.type, rows = 8, title="Intervention type", colors = rev(c("black","#6baed6","#084594","#c6dbef"))))
...
# Country (values by study)
country <- import(paste0(datadir,"allcountries.csv"))
ctry <- data %>% group_by(aid,Study_country) %>%  summarise(n_distinct(aid)) %>%
        left_join(country,by=c("Study_country"="Country")) %>% 
  # quick fix for NAs
        mutate(Region=ifelse(aid==67396,"Latin America",Region),Subregion=ifelse(aid==67396,"Caribbean",Subregion),
...
ctry_sum$ctry[!ctry_sum$ctry%in%country$Country] # any missing?
...
(study_map <- ggplot() +
  ...
  scale_fill_gradient2(low="white",mid="#2171b5",high="#08519c",
                       midpoint=(max(country$n)/2),limits=c(0,max(country$n))))
...
Minor error where some combinations are separated as 2 different chords (e.g.CBNRM-both: Social and Political:1, Political and Social: 2) 
```{r eval=T}
###########################
##FOR MPAS
###########################
...
max.subtypes <- paste0("o.sub",rep(1:max(str_count(outcomes$Out_subtype, ",")+1))) 
outcomes2 <-data %>% 
...
cbnrm_tree_count <- filter(outcomes3,Int_type == "CBNRM") 
cbnrm_mpa_tree_count <- filter(outcomes3,Int_type == "CBNRM_MPA") 
