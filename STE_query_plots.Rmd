...
---
## Background
Code for analysis and plots for Synergies, Tradeoffs and Equity in marine conservation paper.

Some additional edits need to be made to the plots for resubmission:

* Country map (change articles to studies)
* Scale vs unit vs scale type
* Demographic gp vs demography
* CBM to CBNRM in fig 2
* Fig4: all boxes equal size
* Fig2: no hyphen in wellbeing, Capitalize labels
* Fig 6: lose title, change to legend title
...
mapdir <- paste0(pcdir,"Dropbox/SynergiesTradeoffs.Review/Data extraction/basemaps/")
...
fig.outputdir2 <- paste0(pcdir,"Dropbox/SynergiesTradeoffs.Review/Submission materials/resubmission/pdf files/individual figures/")
...
country <- import(paste0(mapdir,"allcountries.csv"))
...
int.type <- data %>% 
  group_by(Int_type) %>% 
  summarise(num=n_distinct(aid), pct=paste0("(",round((num/75)*100)," %)")) %>% 
 # unite("Intervention",Int_type,pct, sep=" ") %>% 
  arrange (-num)
(pInt_typ.waffle <- waffle(int.type, rows = 7, colors = rev(c("black","#6baed6","#084594","#c6dbef"))))
...
# wrangle data
io_counts <- data %>% 
        group_by(Int_type,Out_type) %>% # group data by intervention type and outcome type
        count() %>% # count observations 
        spread(key=Int_type, value=n) %>%  # spread the values (each intervention type in own column, this allows us to find NAs)
    #   mutate(PES=0) %>%
        gather("Int_type","n",CBM:MPA) %>% # gather the values (in order to fill NAs with 0s)
        mutate(n=ifelse(is.na(n),0,as.integer(n))) # fill NAs with 0

(heat.map.domain <- ggplot(data=io_counts, aes(x=Out_type,y=Int_type,fill=n)) +
  geom_tile(color="gray90",size=0.1) +
  geom_text(aes(label=n),show.legend = F) +
  scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90", limits=c(0,40)) +
  coord_equal() +
...
id_counts <- data %>% 
  ...
  mutate(ste_type=gsub("STE_","",ste_type),
         ste_type=str_to_title(ste_type))
...
# theme_tufte(base_family="Helvetica") +    # having issues with font, font colour in windows...
  ...
  theme(legend.key.size=unit(1, "cm")) +
  theme(legend.key.width=unit(1, "cm")) +
  scale_x_discrete(name="Social/demographic group", labels=c("Age","Economic","Ethnicity","Gender","Occupation", "Social")) +
  ylab("Intervention") +
  theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))
...
#  scale_x_discrete(name="Scale", labels=c("spatial units","temporal scale","temporal units")) +
  scale_x_discrete(name="Scale type", labels=c("Spatial","Temporal")) +
  ylab("Intervention") +
  theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))

#separate heat map by S,T,S&T##########################################################

#------ Domains
idom_counts_STE <- data %>% 
  select(Out_type,STE_domain) %>% 
  # mutate_at(vars(STE_econ:STE_social_org),funs(ifelse(is.na(.),0,1))) %>% 
  # group_by(aid) %>% summarise_at(vars(STE_econ:STE_social_org), funs(max(.))) %>% 
  filter(!is.na(STE_domain)) %>% 
  filter(STE_domain%in%c("Synergy","Tradeoff","Both Syn/Trd")) %>% 
  group_by(Out_type,STE_domain) %>% 
  add_count() %>% 
  unique %>% 
  ungroup()


(heat.map.domainSTE <- ggplot(idom_counts_STE, aes(x=Out_type,y=STE_domain,fill=n)) +
  geom_tile(color="gray90",size=0.1) +
  geom_text(aes(label=n),show.legend = F) +
  scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90", limits=c(0,40)) +
  coord_equal() +
# theme_tufte(base_family="Helvetica") +    # having issues with font, font colour in windows...
  ...
  theme(legend.key.size=unit(1, "cm")) +
  theme(legend.key.width=unit(1, "cm")) +
  scale_x_discrete(name="Wellbeing domain") +
  ylab("STE type") +
  theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))

#------ Demographic groups
id_counts_STE <- data %>% 
  select(STE_econ:STE_social_org) %>% 
  # mutate_at(vars(STE_econ:STE_social_org),funs(ifelse(is.na(.),0,1))) %>% 
  # group_by(aid) %>% summarise_at(vars(STE_econ:STE_social_org), funs(max(.))) %>% 
  gather("ste_type","val",STE_econ:STE_social_org) %>% 
  filter(!is.na(val)) %>% 
  filter(val%in%c("Synergy","Tradeoff")) %>% 
  group_by(ste_type,val) %>% 
  add_count() %>% 
  unique %>% 
  ungroup() %>% 
  mutate(ste_type=gsub("STE_","",ste_type),
...
(heat.map.demogSTE <- ggplot(id_counts_STE, aes(x=ste_type,y=val,fill=n)) +
  geom_tile(color="gray90",size=0.1) +
  geom_text(aes(label=n),show.legend = F) +
  scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90", limits=c(0,40)) +
  coord_equal() +
# theme_tufte(base_family="Helvetica") +    # having issues with font, font colour in windows...
  ...
  theme(legend.key.size=unit(1, "cm")) +
  theme(legend.key.width=unit(1, "cm")) +
  scale_x_discrete(name="Social/demographic group", 
                   labels=c("Age","Economic","Ethnicity","Gender","Occupation", "Social")) +
  ylab("STE type") +
  theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))
...
is_counts_STE <- data %>% 
  select(STE_spatial:STE_temporal_scale) %>% 
  gather("ste_type","val",STE_spatial:STE_temporal_scale) %>% 
  mutate(ste_type=gsub("STE_temporal_scale","STE_temporal_unit",ste_type)) %>% 
  filter(!is.na(val)) %>% 
  filter(val%in%c("Synergy","Tradeoff")) %>% 
  group_by(ste_type,val) %>% 
  add_count() %>% 
...
(heat.map.scaleSTE<- ggplot(is_counts_STE, aes(x=ste_type,y=val,fill=n)) +
  geom_tile(color="gray90",size=0.1) +
  geom_text(aes(label=n),show.legend = F) +
  scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90", limits=c(0,40)) +
  coord_equal() +
# theme_tufte(base_family="Helvetica") +    # having issues with font, font colour in windows...
  ...
  theme(legend.key.size=unit(1, "cm")) +
  theme(legend.key.width=unit(1, "cm")) +
  scale_x_discrete(name="Scale type", 
                   labels=c("Spatial","Temporal")) +
  ylab("STE type") +
  theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))

plot_grid(heat.map.domainSTE,heat.map.demogSTE, heat.map.scaleSTE,labels=letters[1:3], ncol = 3, nrow = 1, hjust=-1)
ggsave(paste0(fig.outputdir,'synergies_tradeoff_counts.pdf'),width = 14,height = 5)


#------ All groups
# 
# iall_counts_STE <- data %>% 
#   select(STE_domain:STE_temporal_scale) %>% 
...
#          ste_type=str_to_title(ste_type))
# 
# (heat.map.allSTE<- ggplot(iall_counts_STE, aes(x=ste_type,y=val,fill=n)) +
#   geom_tile(color="gray90",size=0.1) +
#   geom_text(aes(label=n),show.legend = F) +
#   scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90", limits=c(0,40)) +
#   coord_equal() +
...
#   theme(legend.key.width=unit(1, "cm")) +
#   scale_x_discrete(name="Scale type", 
#                    labels=c("Age","Domain","Economic","Ethnicity","Gender","Occupation", "Social","Spatial","Temporal")) +
#   ylab("Intervention") +
#   theme(axis.text.x = element_text(angle=45,hjust=1,size=12)))
...
map <- st_read(paste0(mapdir,"TM_WORLD_BORDERS-0.3/TM_WORLD_BORDERS-0.3.shp")) %>% 
...
heat.map.domain <- ggplot(.data, aes_string(x='source',y='target',fill='n')) +
    geom_tile(color="gray90",size=0.1) +
    geom_text(aes(label=n),show.legend = F) +
    scale_fill_gradient2(low="#f7fbff",high="#2171b5",name="# Cases",na.value="gray90",limits=c(0,20)) +
    coord_equal() +
    #theme_tufte(base_family="Helvetica") +    # having issues with font, font colour in windows...
    theme(axis.ticks=element_line(size=0.4)) +
    theme(axis.text=element_text(size=12)) +
...
mpa.counts.both <- data %>% 
  ...
  link_counts() %>% 
  link_heatmap()
cbmpa.counts.syn <- data %>% 
  filter(Int_type == "CBMPA" & STE_domain == "Synergy") %>% 
  arrange(Out_type,aid) %>% 
  link_counts() %>% 
  link_heatmap()
cbmpa.counts.trdoff <- data %>% 
  filter(Int_type == "CBMPA" & STE_domain == "Tradeoff") %>% 
  arrange(Out_type,aid) %>% 
  link_counts() %>% 
  link_heatmap()
cbmpa.counts.both <- data %>% 
  filter(Int_type == "CBMPA" & STE_domain == "Both Syn/Trd") %>% 
  arrange(Out_type,aid) %>% 
  link_counts() %>% 
  link_heatmap()
cbnrm.counts.syn <- data %>% 
  filter(Int_type %in% c("CBM") & STE_domain == "Synergy") %>% 
  arrange(Out_type,aid) %>% 
  link_counts() %>% 
  link_heatmap()
cbnrm.counts.trdoff <- data %>% 
  filter(Int_type %in% c("CBM") & STE_domain == "Tradeoff") %>% 
  arrange(Out_type,aid) %>% 
  link_counts() %>% 
  link_heatmap()
cbnrm.counts.both <- data %>% 
  filter(Int_type %in% c("CBM") & STE_domain == "Both Syn/Trd") %>% 
  arrange(Out_type,aid) %>%
  link_counts() %>% 
  link_heatmap()
...
ST_heat.map.v <- plot_grid(mpa.counts.syn+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="MPA",title="Synergy"),
          mpa.counts.trdoff+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(title="Tradeoffs"),
          mpa.counts.both+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(title="Both Synergies & Tradeoffs"),
          cbmpa.counts.syn+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="CBMPA"),
          cbmpa.counts.trdoff+theme(legend.position = "none",axis.title=element_blank()), 
          cbmpa.counts.both+theme(legend.position = "none"),
          ...
          labels=letters[1:12], ncol = 3, nrow = 4, hjust=-1)

ST_heat.map.h <- plot_grid(mpa.counts.syn +theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="Synergies",title="MPA"),
                           cbmpa.counts.syn+theme(legend.position = "none")+labs(title="CBMPA"),
                           cbnrm.counts.syn+theme(legend.position = "none")+labs(title="CBM"),
                           cert.counts.syn+theme(legend.position = "none")+labs(title="Certification"),
                           mpa.counts.trdoff+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="Tradeoffs"),
                           cbmpa.counts.trdoff+theme(legend.position = "none"),
                           cbnrm.counts.trdoff,
                           cert.counts.trdff,
                           mpa.counts.both+theme(axis.title.y=element_text(face="bold"))+labs(y="Both Synergies & Tradeoffs"),
                           cbmpa.counts.both,
                           cbnrm.counts.both,
                           cert.counts.both,
          labels=letters[1:12], ncol = 4, nrow = 3, hjust=-1)

ST_heat.map.h.nolegend <- plot_grid(mpa.counts.syn +theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="Synergies",title="MPA"),
                           cbmpa.counts.syn+theme(legend.position = "none")+labs(title="CBMPA"),
                           cbnrm.counts.syn+theme(legend.position = "none")+labs(title="CBM"),
                           cert.counts.syn+theme(legend.position = "none")+labs(title="Certification"),
                           mpa.counts.trdoff+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="Tradeoffs"),
                           ...
                           cbnrm.counts.trdoff,
                           cert.counts.trdff,
                           mpa.counts.both+theme(legend.position = "none",axis.title.y=element_text(face="bold"))+labs(y="Both Synergies & Tradeoffs"),
                           cbmpa.counts.both+theme(legend.position = "none"),
                           cbnrm.counts.both+theme(legend.position = "none"),
                           cert.counts.both+theme(legend.position = "none"),
          labels=letters[1:12], ncol = 4, nrow = 3, hjust=-1)
...
# Syn vs Trad vs Nils
data %>% 
  select(aid,STE_domain) %>% 
  group_by(STE_domain) %>% 
  summarise(n=n_distinct(aid), pct=n/75)
data %>% 
  select(aid,STE_occupation) %>% 
  group_by(STE_occupation) %>% 
  summarise(n=n_distinct(aid), pct=n/75)
data %>% 
  select(aid,STE_spatial) %>% 
  group_by(STE_spatial) %>% 
  summarise(n=n_distinct(aid), pct=n/75)
...
# Heat map
group.list <- gsub("STE_","",names(select(data,STE_econ:STE_occupation)))
...
(pInt_time_gps <- ggplot(Int_time_gps)  + 
  ...
  ylab("Cumulative frequency") + scale_x_continuous(name="Year", breaks=seq(1985,2025,5)) +
...
STE_time_dom <-  data %>%
  ...
  mutate(out_typ=gsub("Economic well-being","Economic wellbeing", out_typ))
tail(STE_time_dom)
...
STE_time_demog <- STE_time %>%
  ...
  mutate(STE_gp=gsub("STE_","",STE_gp),
         STE_gp=gsub("econ","economic",STE_gp),
         STE_gp=str_to_title(STE_gp)) %>% 
...
(pSTE_scale_gps <- ggplot(STE_scale_gps)  + 
  geom_line(aes(x = Pub_year, y = val, group = Type, colour = Type),size=1.3)  +
  ylab("Cumulative frequency") + scale_x_continuous(name="Year", breaks=seq(1985,2025,5)) +
  scale_colour_manual(values=ste.cols) +
  ggtitle('Scale') +
  scale_y_continuous(limits=c(0,80), breaks=seq(0,80,10), labels=c("0","","20","","40","","60","","80")))
...
#### For paper
```{r, EVAL =F}
# Figure 2. Cumulative number of studies 
plot_grid(pInt_time_gps, pSTE_time_dom,pSTE_time_demog,pSTE_scale_gps,
          labels=letters[1:4], ncol = 2, nrow = 2, hjust=-1)
#ggsave(paste0(fig.outputdir,'fig2_study_gps_over_time.jpg'),width = 14,height = 8)
ggsave(paste0(fig.outputdir,'fig2_study_gps_over_time.pdf'),width = 14,height = 8)


# Figure 3. Number of studies by ctry
#png(file=paste0(fig.outputdir,"fig3_Auth_Study_Country_Map_dark.png"), width=13.5, height=16, res=600, units = "in")
pdf(file=paste0(fig.outputdir,"fig3_Auth_Study_Country_Map_dark.pdf"), width=20, height=6)
plot_grid(auth_ctry_map + labs(title="Author insitutions", x="Long", y="Lat"),
          study_ctry_map + labs(title="Study locations", x="Long", y="Lat"),
          labels=letters[1:2], ncol = 2, nrow = 1, hjust=-1)
dev.off()
# Resubmission
study_ctry_map+theme(legend.position="bottom") + labs(title = NULL)

# Fig4. Study design and outcome types 
...
#ggsave(paste0(fig.outputdir,'fig4_STE outcome-data observer.jpg'),width = 14,height = 4)
ggsave(paste0(fig.outputdir,'fig4_STE outcome-data observer.pdf'),width = 14,height = 4)

# Fig5 Outcome types by type of study for 195 social outcomes 
pOut_type.stdytype
#ggsave(paste0(fig.outputdir,'fig5_STE outcome-studytype.jpg'),width = 7,height = 4)
ggsave(paste0(fig.outputdir,'fig5_STE outcome-studytype.pdf'),width = 7,height = 4)

# Fig6. Number of interventions
pInt_typ.waffle 
#ggsave(paste0(fig.outputdir,'fig6_STE intervention waffle.jpg'),width = 8,height = 4)
ggsave(paste0(fig.outputdir,'fig6_STE intervention waffle.pdf'),width = 8,height = 4)

# Fig7. Heat map: outcomes assessed by intervention and a) domain; b) demographic group and; 3) scale
#png(file=paste0(fig.outputdir,"fig7_Heatmap_articles.png"), width=13.5, height=5, res=600, units = "in")
pdf(file=paste0(fig.outputdir,"fig7_Heatmap_articles.pdf"), width=13.5, height=5)
plot_grid(heat.map.domain,heat.map.demog, heat.map.scale,labels=letters[1:3], ncol = 3, nrow = 1, hjust=-1)
dev.off()

# Fig8. Heat maps showing the number of reported synergies (a,d,g), tradeoffs 
ST_heat.map.v
ggsave(paste0(fig.outputdir,"fig8_syn_trd_both_heatmaps.v.pdf"),width = 17,height = 13)
ST_heat.map.h
#ggsave(paste0(fig.outputdir,"fig8_syn_trd_both_heatmaps.png"),width = 13,height = 13)
ggsave(paste0(fig.outputdir,"fig8_syn_trd_both_heatmaps.h.pdf"),width = 17,height = 13)
ST_heat.map.h.nolegend
ggsave(paste0(fig.outputdir,"fig8_syn_trd_both_heatmaps.h.nolegend.pdf"),width = 17,height = 13)


# Fig9. Distribution of domains and sub-domains 
# All interventions
#png(file=paste0(fig.outputdir,"fig9_Outcome_attr_by_domain_all.png"), width=9, height=8, res=600, units = "in")
pdf(file=paste0(fig.outputdir,"fig9_Outcome_attr_by_domain_all.pdf"), width=9, height=8)
treemap(outcomes3,index=c("Out_type_all","Out_subtype"),
        vSize="n",
        type="index",
        fontsize.labels = c(15,10),
