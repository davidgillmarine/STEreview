...
data %>% group_by(auth_ctry1) %>% summarise(num=n_distinct(aid), pct=num/75) %>% arrange(desc(num)) %>% mutate(pct_sum=cumsum(pct))

(pAuth_ctry<-ste_gp(auth_ctry1) +
  xlab("Author institution location"))
...
#pdf(file=paste0(outputdir,"Auth_Country_Map_dark_5_11.pdf"), width=16, height=8.5)
plot_grid(study_ctry_map + labs(title="Study locations"),
          auth_ctry_map + labs(title="Author insitutions"),labels=letters[1:2], ncol = 1, nrow = 2, hjust=-1)
dev.off()
