...
outputdir <- "C:/Users/dgill6/Dropbox/SynergiesTradeoffs.Review/Data extraction/updated_plots/"
...
# Publication type
data %>% group_by(Pub_type) %>% summarise(n_distinct(aid))
(pPubtype<-ste_gp(Pub_type) +
  xlab("Publication type"))

# Author affliation
data %>% group_by(auth_aff) %>% summarise(n_distinct(aid))
(pAuth_aff<-ste_gp(auth_aff) +
  xlab("Author affliation"))

# Intervention type
data %>% group_by(Int_type) %>% summarise(n_distinct(aid))
(pInt_typ<-ste_gp(Int_type) +
  xlab("Intervention type"))

# Intervention scale
data %>% group_by(int_scale) %>% summarise(n_distinct(aid))
...
# Study data source
data %>% group_by(Data_source) %>% summarise(n_distinct(aid))
(pStdy_source<-ste_gp(Data_source) +
  xlab("Study data source"))

# Study scale
data %>% group_by(stdy_scl) %>% summarise(n_distinct(aid))
...
# Biome
data %>% group_by(Biome.) %>% summarise(n_distinct(aid))
(pBiome<-ste_gp(Biome.) +
  xlab("Biome"))

# # Country
# data %>% group_by(Study_country) %>% summarise(n_distinct(aid))
# (pCountry<-ste_gp(Study_country) +
#   xlab("Study country"))
...
test <- strsplit(as.character(ctry$Study_country),'; ',fixed=TRUE)
do.call(rbind, test)
head(test)
...
country <- import(paste0(datadir,"allcountries.csv")) %>% 
  left_join(ctry_sum,by=c("Country"="ctry")) %>%
  mutate(n=replace(n, is.na(n), 0)) 

ctry_sum$ctry[!ctry_sum$ctry%in%country$Country]
# read in map
...
(study_map <- ggplot() +
  ...
  expand_limits(x=map$long,y=map$lat) +
  theme(panel.background = element_rect(fill = "darkgray", colour = "darkgray"), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_fill_gradient2(low="#f7fbff",mid="#6baed6",high="#08306b",midpoint=(max(country$n)/2),limits=c(0,max(country$n))))
...
## about intervention
plot_grid(pInt_typ,pInt_scale,pBiome
          ,labels=letters[1:4], ncol = 2, nrow = 2, hjust=-1)
...
plot_grid(pDatatype,pDataobs,pStudy_typ, pCtrl_gp,labels=letters[1:4], 
          ncol = 2, nrow = 2, hjust=-1)
ggsave(paste0(outputdir,'study design.jpg'),width = 10.5,height = 6)

#-- World map
png(file=paste0(outputdir,"STE_Country_Map_dark.png"), width=10.5, height=8, res=600, units = "in")
...
png(file=paste0(outputdir,"Int_Outcome_Heatmap_articles.png"), width=8, height=6, res=600, units = "in")
...
#-- Chord diagrams
## MPA
#pdf(file=paste0(outputdir,"STE by Domain - MPA_4_24.pdf"),width=11,height=8.5)
par(oma=c(0,0,2,0))
png(file=paste0(outputdir,"STE by Domain-MPA-tradeoffs.png"), width=8, height=6, res=600, units = "in")
#par(mfrow=c(1,2),oma=c(0,0,2,0))
chordDiagram(as.data.frame(mpa.counts.trdoff), transparency = 0.3, grid.col=grid.col)
title("Tradeoffs (n=16 articles, x=46 cases)") 
dev.off()
png(file=paste0(outputdir,"STE by Domain-MPA-both.png"), width=8, height=6, res=600, units = "in")
chordDiagram(as.data.frame(mpa.counts.both), transparency = 0.3, grid.col=grid.col)
title("Both (n=16 articles, x=46 cases)") 
dev.off()
png(file=paste0(outputdir,"STE by Domain-MPA-syn.png"), width=8, height=6, res=600, units = "in")
chordDiagram(as.data.frame(mpa.counts.syn), transparency = 0.3, grid.col=grid.col)
title("Synergies (n=26 articles, x=70 cases)")
dev.off()
...
# chordDiagram(as.data.frame(counts), transparency = 0.3, grid.col=grid.col) # no tradeoff data?
# title(paste0("Tradeoffs (n=",length(aid)," articles, x=",nrow()," cases)"),line=-3.5)
png(file=paste0(outputdir,"STE by Domain-CBNRM-syn.png"), width=8, height=6, res=600, units = "in")
chordDiagram(as.data.frame(cbnrm.counts.syn), transparency = 0.3, grid.col=grid.col)
title("Synergies (n=8 articles, x=21 cases)",line=)
dev.off()
png(file=paste0(outputdir,"STE by Domain-CBNRM-both.png"), width=8, height=6, res=600, units = "in")
chordDiagram(as.data.frame(cbnrm.counts.both), transparency = 0.3, grid.col=grid.col)
title("Both (n=8 articles, x=21 cases)",line=)
mtext("STE by outcome domain type in CBNRM",outer=TRUE,cex=1.5)
dev.off()
...
plot_grid(pInt_time_gps, pSTE_time_dom,pSTE_time_demog,pSTE_scale_gps,
          labels=letters[1:4], ncol = 2, nrow = 2, hjust=-1)
ggsave(paste0(outputdir,'study_gps_over_time.jpg'),width = 10.5,height = 6)

# main groups over time
pStudy_typ_time
ggsave(paste0(outputdir,'study_type_over_time.jpg'),width = 11,height = 5)
