...
data %>% filter(grepl("food security",Out_subtype)) %>% group_by(cpue) %>% summarise(num=n_distinct(aid), pct=num/24) %>% arrange(desc(num))
  ...
  # Studies focused on fishers
fisher.terms <-   c(grep("fish",data$Out_subgroup,value=T), grep("CPUE",data$Out_subgroup,value=T,ignore.case = F),
                  grep("co-op",data$Out_subgroup,value=T,ignore.case = F), grep("coop",data$Out_subgroup,value=T,ignore.case = F),
                  grep("gear",data$Out_subgroup,value=T))
data %>% 
  mutate(fisher =ifelse(Out_subgroup%in%fisher.terms & !is.na(STE_occupation),1,0)) %>%
  filter(Out_subgroup%in%fisher.terms & !is.na(STE_occupation)) %>% 
  group_by(fisher) %>% 
  summarise(num=n_distinct(aid), 
            pct=num/length(unique(data$aid[!is.na(data$STE_occupation)])))
